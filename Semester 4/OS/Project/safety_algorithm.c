...
SafetyResult run_safety_algorithm(void) {
    ...
    while (count < sys.num_processes) {
        ...
        for (int i = 0; i < sys.num_processes; i++) {
            ...
            if (can_execute) {
                /* Process i can execute to completion */
                for (int j = 0; j < sys.num_resources; j++) {
        ...
        if (!found) {
            /* No process could execute - unsafe state */
            break;
        }
    }
 ...
 * This is used during request handling to test if granting a request keeps system safe.
 */
bool check_safety_with_temp(int temp_available[], int temp_allocation[][MAX_RESOURCES], 
    ...
    /* Verify all active processes finished */
    for (int i = 0; i < num_p; i++) {
